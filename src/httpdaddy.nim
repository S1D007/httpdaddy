...
type
  ...
  httpdaddyDefect* = ref object of Defect

const
  serverInfo = "httpdaddy"
...
proc eventLoop(
  ...
  if compileOption("threads") and not settings.reusePort:
    raise httpdaddyDefect(msg: "--threads:on requires reusePort to be enabled in settings")
  ...
  try:
    ...
    if e.errorCode == ENOPROTOOPT:
      echo(
        "SO_REUSEPORT not supported on this platform. httpdaddy will not utilise all threads."
      )
    else: raise
...
proc send*(req: Request, code: HttpCode, body: string, headers="") =
  ...
  withRequestData(req):
    ...
    if requestData.requestID != req.requestID:
      raise httpdaddyDefect(msg: "You are attempting to send data to a stale request.")
...
proc forget*(req: Request) =
  ## Unregisters the underlying request's client socket from httpdaddy's
  ## event loop.
  ##
  ## This is useful when you want to register ``req.client`` in your own
  ## event loop, for example when wanting to integrate httpdaddy into a
  ## websocket library.
...
when false:
  proc close*(port: Port) =
    ## Closes an httpdaddy server that is running on the specified port.
    ##
